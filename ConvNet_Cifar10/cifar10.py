...
# w的初始化函数,添加weight_decay进行w的L2正则化 
...
def loss(logits, labels):
  # 强制类型转换，使符合sparse_softmax_cross_entropy_with_logits输入参数格式要求
  ...
  # 把张量cross_entropy_loss添加到字典集合中key='losses'的子集中  
...
def train(total_loss, global_step):
  ...
  # tf.control_dependencies是一个context manager，控制节点执行顺序
  # 先执行control_inputs中的操作，再执行context中的操作
