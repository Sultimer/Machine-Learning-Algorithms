...
if __name__ == '__main__':
    ...
    # 输出原始照片
#    img = np.reshape(X_train[1], (32, 32, 3)) 
#    imageio.imwrite("cifar1.jpg", img)
    ...
    # 降维（可注释）
    pca = PCA()
    X_train, X_test = pca.reduce_dimension(X_train, X_test)
#    reconMat = pca.reduce_dimension(X_train, X_test)
    
    # 输出pca降维后照片
#    img = np.reshape(reconMat[1], (32, 32, 3)) 
#    imageio.imwrite("cifar2.jpg", img)
    
    lda = LDA()
    X_train, X_test = lda.train(X_train, X_test, y_train, 6)
    k = 100
    ...
    accuracy = np.sum(y_te_pred == y_te) / float(X_te.shape[0])
    print('最终测试： K = %d, accuracy = %.3f' % (k, accuracy))
